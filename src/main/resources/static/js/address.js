...
var address = {
    init: function (options) {
        $.extend(addressCfg, options);
        var root = this;
        var qxzHtml = '<option value="0">请选择</option>';
        addressCfg.provinceElement.html(root.getOptionHtml(area_array));
        addressCfg.areaElement.html(qxzHtml);
        addressCfg.cityElement.html(qxzHtml);

        addressCfg.provinceElement.change(function () {
            addressCfg.areaElement.show();
            if ($(this).val() == 0) {
                addressCfg.cityElement.html(qxzHtml);
                addressCfg.areaElement.html(qxzHtml);
                return;
            }
            var optionHtml = root.getOptionHtml(sub_array[parseInt($(this).val())]);
            addressCfg.cityElement.html(optionHtml);

            var array = sub_array[parseInt($(this).val())];
            var j = 0;
            for (var i = 0; i < array.length; i++) {
                if (typeof array[i] !== "undefined" && typeof  sub_arr[i] !== "undefined") {
                    for (var l = 0; l < sub_arr[i].length; l++) {
                        j++;
                        break;
                    }
                }
            ...
            if (j > 0) {
                ...
                addressCfg.areaElement.hide();
            }
        });

        addressCfg.cityElement.change(function () {
            if ($(this).val() == 0) {
                addressCfg.areaElement.html(qxzHtml);
                return;
            }
            if (typeof sub_arr[parseInt($(this).val())] === "undefined") {
                var optionHtml = '<option value="0">---</option>';
                addressCfg.areaElement.hide();
            } else {
                addressCfg.areaElement.show();
                var optionHtml = root.getOptionHtml(sub_arr[parseInt($(this).val())]);
            }
            addressCfg.areaElement.html(optionHtml);

        });

    },
    getOptionHtml: function (array) {
        var arrayHtml = [];

        for (var i = 0; i < array.length; i++) {
            if (typeof array[i] !== "undefined") {
                arrayHtml.push('<option value="' + i + '">' + array[i] + '</option>');
            }
        }
        return arrayHtml.join("");
    },
    query: function (province, city, area) {
        var array = area_array;
        var arrayHtml = [];
        var arrayHtml2 = [];
        ...
        for (var i = 0; i < array.length; i++) {
            if (typeof array[i] !== "undefined") {
                if (array[i] === province) {
                    provinceId = i;
                    arrayHtml.push('<option value="' + i + '" selected>' + array[i] + '</option>');
                } else {
                    arrayHtml.push('<option value="' + i + '">' + array[i] + '</option>');
                }
            }
        ...
        for (var i = 0; i < sub_array[provinceId].length; i++) {
            if (typeof sub_array[provinceId][i] !== "undefined") {
                if (sub_array[provinceId][i] === city) {
                    cityId = i;
                    arrayHtml2.push('<option value="' + i + '" selected>' + sub_array[provinceId][i] + '</option>');

                } else {
                    arrayHtml2.push('<option value="' + i + '">' + sub_array[provinceId][i] + '</option>');
                }
            }
        }
        ...
        if (area && area !== "") {
            ...
            for (var i = 0; i < sub_arr[cityId].length; i++) {
                if (typeof sub_arr[cityId][i] !== "undefined") {
                    if (sub_arr[cityId][i] === area) {
                        arrayHtml3.push('<option value="' + i + '" selected>' + sub_arr[cityId][i] + '</option>');
                    } else {
                        arrayHtml3.push('<option value="' + i + '">' + sub_arr[cityId][i] + '</option>');
                    }
                }
            }
            addressCfg.areaElement.show();
            addressCfg.areaElement.html(arrayHtml3.join(""));
        } else {
            var arrayHtml3 = this.getOptionHtml(sub_arr[cityId]);
            addressCfg.areaElement.show();
            addressCfg.areaElement.html(arrayHtml3);
        }

        addressCfg.provinceElement.html(arrayHtml.join(""));
        addressCfg.cityElement.html(arrayHtml2.join(""));
